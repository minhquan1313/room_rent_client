export const routeList = "all";
export const routeAdd = "add";
export const routeUpdate = "edit";
export const routeDelete = "delete";

export const routeRoomEdit = "/rooms/edit";
export const routeRoomDetail = "/rooms/detail";
export const routeRoomAdd = "/rooms/add";
export const routeRoomSearch = "/rooms/search";
export const routeChat = "/chat";
export const routeUserDetail = "/users";
export const routeUserEditSelf = "/users/edit";

export const routeFavoriteRoom = "/bookmark/rooms";

export const routeAdmin = {
  index: "/dashboard",

  stats: "stats",

  user: "users",
  // userAdmin: "users/admin",
  // userAdmin2: "users/admin2",
  // userOwner: "users/owner",
  // userUser: "users/user",

  room: "rooms",
  roomList: "room-all",
  roomService: "room-services",
  roomType: "room-types",
};
